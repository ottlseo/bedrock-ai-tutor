...
const startStreaming = async ({
    ...
    const recordingprops = {
      ...
      const tmediaRecorder = new AudioWorkletNode(
        audioContext,
        'recording-processor',
        {
      ...
      tmediaRecorder.port.postMessage({
        message: 'UPDATE_RECORDING_STATE',
        setRecording: true,
      });
    
      source1.connect(tmediaRecorder).connect(destination);
      tmediaRecorder.port.onmessageerror = (error) => {
        console.log(`Error receving message from worklet ${error}`);
      };
    
      const audioDataIterator = pEvent.iterato(tmediaRecorder.port, 'message');
      ...
      const tdata = await transcribeClient.send(command);
      console.log('Transcribe sesssion established ', tdata.SessionId);
    
    
      if (tdata.TranscriptResultStream) {
        for await (const event of tdata.TranscriptResultStream) {
